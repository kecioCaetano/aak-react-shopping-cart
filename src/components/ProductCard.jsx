...
function ProductCard(prop) {
    ...
    return (
        <Link to={`/products/${content.type === 'frames' ? 'eyeglasses' : 'sunglasses'}/${content.id}`}
            className="pd-card"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}>
            <div className="pd-card__img--wrapper">
                {content.quantity < 1 && (<div className="pd-card__sold-out">SOLD OUT</div>)}
                {content.onSale && (<div className="pd-card__sale">SALE</div>)}
                <ProgressiveImage
                    src={isHover ? content.images.side : content.images.main}
                    placeholder={isHover ? content.compressedImages.side : content.compressedImages.main}>
                    {(src, loading) => (
                        <img
                            src={src}
